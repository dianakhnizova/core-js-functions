...
function getArgumentsCount(funcs) {
  ...
  return funcs.map((item) => (typeof item === 'function' ? item.length : 0));
}
...
function getPowerFunction(exponent) {
  return function mathPowFunc(number) {
...
function logger(func, logFunc) {
  const myFunc = func;
  const myLogFunc = logFunc;
  return function myLogger(...args) {
    const argsStr = `${args.map((item) => JSON.stringify(item)).join(',')}`;
    const str = `${myFunc.name}(${argsStr}) starts`;
    myLogFunc(str);

    const result = myFunc(...args);

    const str2 = `${myFunc.name}(${argsStr}) ends`;
    myLogFunc(str2);
...
function partialUsingArguments(fn, ...args1) {
  return function myPartialFunc(...args2) {
...
function getIdGeneratorFunction(startFrom) {
  ...
  return function newFuncForGenerateID() {
